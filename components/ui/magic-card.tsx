...
export function MagicCard(props: MagicCardProps) {
  ...
  const glowFrom = isOrbMode(props) ? (props.glowFrom ?? "#008080") : "#008080"
  const glowTo = isOrbMode(props) ? (props.glowTo ?? "#F4A261") : "#F4A261"
  const glowAngle = isOrbMode(props) ? (props.glowAngle ?? 90) : 90
  const glowSize = isOrbMode(props) ? (props.glowSize ?? 500) : 500
  const glowBlur = isOrbMode(props) ? (props.glowBlur ?? 70) : 70
  const glowOpacity = isOrbMode(props) ? (props.glowOpacity ?? 0.7) : 0.7
  ...
  const orbX = useSpring(mouseX, { stiffness: 500, damping: 50, mass: 0.3 })
  const orbY = useSpring(mouseY, { stiffness: 500, damping: 50, mass: 0.3 })
  const orbVisible = useSpring(0, { stiffness: 500, damping: 40 })
